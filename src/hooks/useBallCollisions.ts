  ...
  const handleCollisions = (balls: BallData[]): BallData[] => {
    ...
    for (let i = 0; i < updated.length; i++) {
      for (let j = i + 1; j < updated.length; j++) {
        ...
        if (b1.isProcessing || b2.isProcessing || b1.hasFused || b2.hasFused) continue;
        ...
        if (distance > 0 && distance < minDist) {
          if (fusionEnabled) {
            ...
            const newBall: BallData = {
              ...
              imgUrl: undefined,     // shimmer until real image
              x: (b1.x + b2.x) / 2,
              y: (b1.y + b2.y) / 2,
              ...
              hasFused: true,
              isProcessing: true,
              isSimmering: true,     // ← new flag for shimmer
            };

            // Mark old balls as fused
            b1.hasFused = true;
            b2.hasFused = true;
            ...
            return updated; // handle one fusion per frame
          } else {
            // --- Bounce off behavior ---
            const tempDx = b1.dx;
            const tempDy = b1.dy;
